...
1: Nhập trình cắm
2: Đi tới trang Mitm để kích hoạt Mitm qua Http2
...
if (reg1.test($request.url)) {
    ...
    if ($persistentStore.read('request_id') !== null) {
      $notification.post('ducwtaif thu thập thông tin', 'Thu thập thông tin thành công, vui lòng nhập lệch!','')

    } else {
      $notification.post('ducwtaif thu nhập thông tin TestFlight','Việc thu thập thông tin không thành công, vui lòng bật công tắc Mitm qua HTTP2 và khởi động lại VPN và Ứng dụng TestFlight!','')
    }
    $done({})
}
if (reg2.test($request.url)) {
  ...
  $notification.post("TestFlight tự động tham gia", `Đã thêm: ${id}`, `Đã có sẵn: ${appId}`);
  $done({})
}
function unique(arr) {
